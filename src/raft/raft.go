...
type Raft struct {
	...
	latestHeardTime int64 //最新收到leader的AppendEntries RPC(包括heartbeat)或给予candidate的RequestVote RPC投票的时间
...
type AppendEntriesReply struct {
	...
	ConflictFirstIndex int  //冲突的第一个位置
	Success            bool //标志是否一致化成功
}
...
// AppendEntries 同步最新的命令，让集群所有的日志保持同步，比如发送新的log
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	rf.mu.Lock()
	//当CurrentTerm >term,时回复false，否则继续进行一致性检查
	if rf.CurrentTerm <= args.Term {
		if rf.CurrentTerm < args.Term {
			log.Printf("AppendEntries: Id %d Term %d State %s\t||\targs's term %d is newer\n", rf.me, rf.CurrentTerm, state2name(rf.state), args.Term)
		...
		if len(rf.Log) > args.PrevLogIndex && rf.Log[args.PrevLogIndex].Term == args.PrevLogTerm {
			...
			for i := 0; isMatch && i < len(args.Entries); i++ {
				...
				if end < nextIndex+i {
					isMatch = false
				} else if rf.Log[nextIndex+1].Term != args.Entries[i].Term {
					isMatch = false
				}
			}
			...
			log.Printf("AppendEntries: Id %d Term %d State %s\t||\tcommitIndex %d while leaderCommit %d"+" for leader %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), rf.commitIndex, args.LeaderCommit, args.LeaderId)
			...
			if args.LeaderCommit > rf.commitIndex {
				...
				log.Printf("AppendEntries: Id %d Term %d State %s\t||\tconsistency check pass for index %d"+" with args's prevLogIndex %d args's prevLogTerm %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), indexOfLastOfNewEntry, args.PrevLogIndex, args.PrevLogTerm)
				...
				log.Printf("AppendEntries: Id %d Term %d State %s\t||\tconsistency check failed for index %d"+" with args's prevLogIndex %d args's prevLogTerm %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), index, args.PrevLogIndex, args.PrevLogTerm)
				...
				if len(rf.Log) < nextIndex {
					...
					log.Printf("AppendEntries: Id %d Term %d State %s\t||\tLog's len %d"+" is shorter than args's prevLogIndex %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), len(rf.Log), args.PrevLogIndex)
					log.Printf("AppnedEntries: Id %d Term %d State %s\t||\treply's conflictFirstIndex %d"+" and conflictTerm is None\n", rf.me, rf.CurrentTerm, state2name(rf.state), reply.ConflictFirstIndex)
				} else {
					reply.ConflictTerm = rf.Log[args.PrevLogIndex].Term
					reply.ConflictFirstIndex = args.PrevLogIndex
					log.Printf("AppendEntries: Id %d Term %d State %s\t||\tconsistency check failed"+" with args's prevLogIndex %d args's prevLogTerm %d while it's prevLogIndex %d in"+" prevLogTerm %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), args.PrevLogIndex, args.PrevLogTerm, args.PrevLogIndex, rf.Log[args.PrevLogIndex].Term)
					// 递减reply.ConflictFirstIndex直到index为log中第一个term为reply.ConflictTerm的entry
					for i := reply.ConflictFirstIndex; i >= 0; i++ {
					...
					log.Printf("AppendEntries: Id %d Term %d State %s\t||\treply's conflictFirstIndex %d"+" and conflictTerm %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), reply.ConflictFirstIndex, reply.ConflictTerm)
				}
			}
		}
	}
...
type RequestVoteArgs struct {
	...
	LastLogIndex int //candidate last log entry index
	LastLogTerm  int //candidate last log entry
}
...
type RequestVoteReply struct {
	// Your data here (2A).
	Term        int  //currentTerm
	VoteGranted bool //true 代表candidate收到了选票
}

// RequestVote 向其他节点请求投票，拥有更大的任期和最新的数据的节点会成为candidate并向得到其他节点的选票，
// 没有最新数据的节点会自动转换成Follower
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	//如果term<currentTerm，回复false，否则继续进行选举
	if rf.CurrentTerm <= args.Term {
		//当有节点发现自己的term小于其他节点的，该节点更新自己的任期至其他节点中更大的任期
		if rf.CurrentTerm < args.Term {
			log.Printf("RequestVote: Id %d Term %d State %s\t||\targs's term %d is larger\n", rf.me, rf.CurrentTerm, state2name(rf.state), args.Term)
			rf.CurrentTerm = args.Term
		...
		if rf.VoteFor == -1 || rf.VoteFor == args.CandidateId {
			//通过比较两个日志last entries的index和term来确定哪一个是最新的
			lastLogIndex := len(rf.Log) - 1
			if lastLogIndex < 0 {
				log.Printf("RequestVote: Id %d Term %d State %s\t||\tinvalid lastLogIndex: %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), lastLogIndex)
			}
			lastLogTerm := rf.Log[lastLogIndex].Term
			log.Printf("RequestVote: Id %d Term %d State %s\t||\tlastLogIndex %d and lastLogTerm %d while args's lastLogIndex %d lastLogTerm %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), lastLogIndex, lastLogTerm, args.LastLogIndex, args.LastLogTerm)

			//如果不同任期的log中有相同的last entries那么任期更大的任期中的last entries是更新的
			//如果在同一任期中的log中log更长的是最新的
			if lastLogTerm < args.LastLogTerm ||
				...
				rf.ResetElectionTimer()
				rf.switchTo(Follower)
				log.Printf("[RequestVote]: Id %d Term %d State %s\t||\tgrant vote for candidate %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), args.CandidateId)
				// 授予投票，更新下持久状态
				rf.persist()
				reply.Term = rf.CurrentTerm
...
func (rf *Raft) ResetElectionTimer() {
	...
	rf.electionTimeout = rf.heartbeatPeriod*5 + rand.Intn(150)
	// 因为重置了选举超时，所以也需要更新latestHeardTime
	rf.latestHeardTime = time.Now().UnixNano()
}

// electionTimeoutTick 选举超时(心跳超时)检查器，定期检查自最新一次从leader那里收到AppendEntries RPC(包括heartbeat)
	...
	for {
		...
		if term, isLeader := rf.GetState(); isLeader {
			...
			if int(elapseTime/int64(time.Millisecond)) >= rf.electionTimeout {
				log.Printf("ElectionTimeoutTick: Id %d Term %d State %s\t||\ttimeout, convert to Candidate\n", rf.me, term, state2name(rf.state))
...
// heartbeatPeriodTick 心跳发送周期检查器。leader检查距离上次发送心跳的时间(latestIssueTime)
// 是否超过了心跳周期(heartbeatPeriod)， 若超过则写入数据到heartbeatPeriodChan，以通知发送心跳
	...
	for {
		...
		if term, isLeader := rf.GetState(); isLeader == false {
			...
			if int(elapseTime/int64(time.Millisecond)) >= rf.heartbeatPeriod {
				log.Printf("HeartbeatPeriodTick: Id %d Term %d State %s\t||\theartbeat period elapsed,issue heartbeat\n", rf.me, term, state2name(rf.state))
...
func (rf *Raft) eventLoop() {
	for {
		...
		case <-rf.electionTimeoutChan:
			rf.mu.Lock()
			log.Printf("EventLoop: Id %d Term %d State %s\t||\telection timeout, start an election\n", rf.me, rf.CurrentTerm, state2name(rf.state))
			rf.mu.Unlock()
			go rf.startElection()
		case <-rf.heartbeatPeriodChan:
			rf.mu.Lock()
			log.Printf("EventLoop: Id %d Term %d State %s\t||\theartbeat period occurs, broadcast heartbeats\n", rf.me, rf.CurrentTerm, state2name(rf.state))
			rf.mu.Unlock()
...
func (rf *Raft) applyEntries() {
	for {
		...
		log.Printf("applyEntries: Id %d Term %d State %s\t||\tlastApplied %d and commitIndex %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), lastApplied, commitIndex)
		rf.mu.Unlock()
...
func (rf *Raft) startElection() {
	...
	// 转化为candidate，保存持久状态
	rf.persist()
	log.Printf("StartElection: Id %d Term %d State %s\t||\tstart an election\n", rf.me, rf.CurrentTerm, state2name(rf.state))
	rf.mu.Unlock()
	...
	go func(nVotes *int, rf *Raft) {
		...
		for i, _ := range rf.peers {
			...
			lastLogIndex := len(rf.Log) - 1
			if lastLogIndex < 0 {
				log.Printf("StartElection: Id %d Term %d State %s\t||\tinvalid lastLogIndex %d\n",
					rf.me, rf.CurrentTerm, state2name(rf.state), lastLogIndex)
			}
			...
			log.Printf("StartElection: Id %d Term %d State %s\t||\tissue RequestVote RPC to peer %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), i)
			rf.mu.Unlock()
			...
			go func(i int, rf *Raft, args *RequestVoteArgs, reply *RequestVoteReply) {
				...
				if ok == false {
					rf.mu.Lock()
					log.Printf("StartElection: Id %d Term %d State %s\t||\tsend RequestVote Request to peer %d failed\n", rf.me, rf.CurrentTerm, state2name(rf.state), i)
					rf.mu.Unlock()
					return
				}
				...
				if reply.VoteGranted == false {
					rf.mu.Lock()
					defer rf.mu.Unlock()
					log.Printf("StartElection: Id %d Term %d State %s\t||\tRequestVote is rejected by peer %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), i)
					// convert to follower
					if rf.CurrentTerm < reply.Term {
						log.Printf("StartElection: Id %d Term %d State %s\t||\tless than peer %d Term %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), i, reply.Term)
						rf.CurrentTerm = reply.Term
					...
					// 获得了peer的投票
					rf.mu.Lock()
					log.Printf("StartElection: Id %d Term %d State %s\t||\tpeer %d grants vote\n", rf.me, rf.CurrentTerm, state2name(rf.state), i)
					*nVotes += 1
					log.Printf("StartElection: Id %d Term %d State %s\t||\tnVotes %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), *nVotes)
					// 如果已经获得了多数投票，并且是Candidate状态，则切换到leader状态
					if rf.state == Candidate && *nVotes >= winThreshold {
						log.Printf("StartElection: Id %d Term %d State %s\t||\twin election with nVotes %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), *nVotes)
						// 切换到leader状态
						rf.switchTo(Leader)
						rf.leaderId = rf.me
...
func (rf *Raft) broadcastAppendEntries(index int, term int, commitIndex int, nReplica int, name string) {
	...
	rf.mu.Lock()
	log.Printf("%s: Id %d Term %d State %s\t||\tcreate an goroutine for index %d term %d commitIndex %d to issue parallel and wait\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, term, commitIndex)
	rf.mu.Unlock()
	...
	for i, _ := range rf.peers {
		...
		retry:
			...
			prevLogIndex := nextIndex - 1
			if prevLogIndex < 0 {
				log.Printf("[%s]: Id %d Term %d State %s\t||\tinvalid prevLogIndex %d for index %d peer %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), prevLogIndex, index, i)
			}
			prevLogTerm := rf.Log[prevLogIndex].Term
			...
			if nextIndex > index+1 {
				log.Printf("[%s]: Id %d Term %d State %s\t||\tinvalid nextIndex %d while index %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), nextIndex, index)
			}
			...
			log.Printf("[%s]: Id %d Term %d State %s\t||\tissue AppendEntries RPC for index %d to peer %d with commitIndex %d nextIndex %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i, commitIndex, nextIndex)
			rf.mu.Unlock()
			var reply AppendEntriesReply
			...
			if ok == false {
				rf.mu.Lock()
				log.Printf("%s: Id %d Term %d State %s\t||\tissue AppendEntries RPC for index %d to peer %d failed\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i)
				rf.mu.Unlock()
				return
			}
			...
			if reply.Success == false {
				rf.mu.Lock()
				log.Printf("%s: Id %d Term %d State %s\t||\tAppendEntries RPC for index %d is rejected by peer %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i)
				...
				if args.Term < reply.Term {
					...
					log.Printf("%s: Id %d Term %d State %s\t||\tAppendEntires RPC for index %d is rejected by peer %d due to newer peer's term %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i, reply.Term)
					...
					rf.nextIndex[i] = nextIndex
					log.Printf("%s: Id %d Term %d State %s\t||\tAppendEntries RPC for index %d is rejected by peer %d due to the consistency check failed\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i)
					log.Printf("%s: Id %d Term %d State %s\t||\treply's conflictFirstIndex %d and conflictTerm %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), reply.ConflictFirstIndex, reply.ConflictTerm)
					log.Printf("%s: Id %d Term %d State %s\t||\tretry AppendEntries RPC with nextIndex %d, so prevLogIndex %d and prevLogTerm %d\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), nextIndex, nextIndex-1, rf.Log[nextIndex-1].Term)
					rf.mu.Unlock()
					goto retry
				}
			} else { // AppendEntries RPC发送成功
				rf.mu.Lock()
				log.Printf("%s: Id %d Term %d State %s\t||\tsend AppendEntries RPC for index %d to peer %d success\n",
					name, rf.me, rf.CurrentTerm, state2name(rf.state), index, i)
				...
				if isAgree == false && rf.state == Leader && nReplica >= majority {
					...
					if rf.commitIndex < index && rf.Log[index].Term == rf.CurrentTerm {
						...
						rf.applyCond.Broadcast()
						log.Printf("[%s]: Id %d Term %d State %s\t||\tapply updated commitIndex %d to applyCh\n", name, rf.me, rf.CurrentTerm, state2name(rf.state), rf.commitIndex)
					}
				}
				rf.mu.Unlock()
			}
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if term, isLeader = rf.GetState(); isLeader {
		...
		index = len(rf.Log) - 1
		log.Printf("Start: Id %d Term %d State %s\t||\treplicate the command to Log index %d\n", rf.me, rf.CurrentTerm, state2name(rf.state), index)
		nReplica := 1
		...
		go func(nReplica *int, index int, commitIndex int, term int) {
			...
			rf.mu.Lock()
			log.Printf("Start: Id %d Term %d State %s\t||\tcreate an goroutine for index %d to issue parallel and wait\n", rf.me, rf.CurrentTerm, state2name(rf.state), index)
			rf.mu.Unlock()
